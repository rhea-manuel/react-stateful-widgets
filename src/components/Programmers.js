...
export default function Programmers() {
  ...
  const [programmers, changeAllProgrammers] = useState([...listOfAwesome])
  ...
  const getNameOfFeatured = () => {
    ...
    const toReturn = programmers.find(curProgrammer => {
      return curProgrammer.id == featured
    })
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { changeFeatured(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>

...
export default function Squares() {
  ...
  const [squares, changeAllSquares] = useState([...listOfSquareIds])
  const [activeSquare, changeActiveSquare] = useState(null)
  ...
  const markActive = id => {
    ...
    if (activeSquare !=id){
      changeActiveSquare(id)
    }

    else {
      changeActiveSquare(null)
    }

    // if (activeSquare==id){

    // }
    // if activeSquare.equals(id{
